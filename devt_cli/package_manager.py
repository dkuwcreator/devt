...
class PackageManager:
    ...
    def import_package(
        ...
        if source.is_file() and source.suffix in [".json", ".yaml", ".yml"]:
            try:
                dest = self.move_package_to_tools_dir(source.parent, effective_group)
                if dest:
            ...
            if manifest:
                try:
                    dest = self.move_package_to_tools_dir(source, effective_group)
                    if dest:
                        pkg = PackageBuilder(dest, effective_group).build_package()
                        packages.append(pkg)
                except Exception as e:
                ...
                for mf in source.rglob("manifest.*"):
                    try:
                        dest = self.move_package_to_tools_dir(mf.parent, effective_group)
                        if dest:
                            pkg = PackageBuilder(dest, effective_group).build_package()
                            packages.append(pkg)                 
                    except Exception as e:
