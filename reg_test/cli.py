...
from devt.utils import find_file_type, get_execute_args, load_json
from devt.registry_manager import RegistryManager, get_tool, ToolRepo, guess_repo_name_from_url
...
def list_tools(
    ...
    found_tools = []
    if user_level:
        found_tools.extend(load_registry(USER_REGISTRY_DIR / REGISTRY_FILE_NAME, "user"))
    if workspace_level:
        found_tools.extend(load_registry(WORKSPACE_REGISTRY_DIR / REGISTRY_FILE_NAME, "workspace"))
...
def show_tool_info(
    ...
    """
    workspace_tool = find_tool_in_registry(tool_name, WORKSPACE_REGISTRY_DIR / REGISTRY_FILE_NAME)
    if workspace_tool:
        found_tool = workspace_tool
        scope = "workspace"
    else:
        user_tool = find_tool_in_registry(tool_name, USER_REGISTRY_DIR / REGISTRY_FILE_NAME)
        if user_tool:
